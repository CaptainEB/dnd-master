...
import { createMerchant } from '../../admin/components/actions';

export default function CreateMerchantDialog({ open, onOpenChange, session, onMerchantCreated }) {
	const form = useForm({
		defaultValues: {
			name: '',
			city: '',
	...
	const onSubmit = async (data) => {
		try {
			const result = await createMerchant(session.user.activeCampaignId, data);
			if (result.success) {
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
						...
						<div className="flex justify-end gap-2 pt-4">
							...
							>
								{form.formState.isSubmitting ? 'Creating...' : 'Create Merchant'}
							</Button>
						</div>
					</form>
				</Form>
