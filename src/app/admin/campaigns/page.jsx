			...
			<div className="container mx-auto px-4 py-8">
				<div className="mb-8">
					...
					<div className="flex items-center justify-between">
						<div>
							<h1 className={`text-3xl font-bold mb-2 ${session?.user?.darkMode ? 'text-white' : 'text-gray-900'}`}>Campaign Management</h1>
							<p className={session?.user?.darkMode ? 'text-gray-300' : 'text-gray-600'}>Manage all campaigns across the platform ({campaigns.length} total)</p>
						</div>
						<CreateCampaignForm />
					</div>
				</div>
						...
						campaigns.map((campaign) => {
							...
							return (
								<Link key={campaign.id} href={`/admin/campaigns/${campaign.id}`}>
									<Card className={`p-6 transition-all duration-200 cursor-pointer border-0 shadow-lg backdrop-blur-sm hover:shadow-xl ${
										...
										<div className="flex items-start justify-between mb-4">
											<div>
												...
												<p className={`mb-3 ${session?.user?.darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{campaign.description || 'No description provided'}</p>
											...
											<div className={`mt-4 pt-4 border-t ${session?.user?.darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
												...
												<div className="flex flex-wrap gap-2">
													...
													{campaign.members.length > 5 && (
														...
														}`}>+{campaign.members.length - 5} more</span>
													)}
												</div>
											</div>
										)}
